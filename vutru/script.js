...
const defaultHeartImages = Array.from({ length: 71 }, (_, i) => `images/img${i + 1}.jpg`);
...
const ringTexts = [
  'Cô cho NGUYỄN BẢO KHANG 10 ĐIỂM HÓA nha cô 🥰 ',
  "Mãi yêu cô!",
  "Chúc cô Vương luôn mạnh khỏe, hạnh phúc và thành công trong sự nghiệp trồng người!",
  "20/11/2025 THANH XUÂN VƯỜN TRƯỜNG CỦA CHÚNG ",
...
function initPlaylist(startIndex = null) {
  playlist = audioSources.map((_, i) => i);
  shuffleArray(playlist);
  if (startIndex !== null && playlist.includes(startIndex)) {
    // Đảm bảo track bắt đầu ở đầu playlist
    const pos = playlist.indexOf(startIndex);
    if (pos > 0) {
      playlist.splice(pos, 1);
      playlist.unshift(startIndex);
    }
  }
  playlistPos = 0;
}
...
function cleanupCurrentAudio() {
  ...
  galaxyAudio.removeEventListener('ended', onTrackEnded);
  try { galaxyAudio.pause(); } catch (e) {}
  try { galaxyAudio.src = ""; } catch (e) {}
  galaxyAudio = null;
}

function createAudioForCurrent() {
  cleanupCurrentAudio();
  if (!playlist || playlist.length === 0) initPlaylist();
  const srcIndex = playlist[playlistPos % playlist.length];
  const src = audioSources[srcIndex];
  galaxyAudio = new Audio(src);
  galaxyAudio.preload = "auto";
  galaxyAudio.loop = false; // quản lý loop qua playlist
  galaxyAudio.volume = 1.0;
  galaxyAudio.addEventListener('ended', onTrackEnded);
  galaxyAudio.addEventListener('error', (e) => console.warn('Galaxy audio error', e));
}

function onTrackEnded() {
  playlistPos++;
  if (playlistPos >= playlist.length) {
    // khi playlist chạy hết -> xáo lại và bắt đầu từ đầu
    initPlaylist();
  }
  createAudioForCurrent();
  playGalaxyAudio();
}

/**
 * Giữ tên hàm cũ để không phá logic khác trong project.
 * preloadGalaxyAudio(startIndex) sẽ tạo playlist và preload track đầu.
 */
function preloadGalaxyAudio(startIndex = Math.floor(Math.random() * audioSources.length)) {
  initPlaylist(startIndex);
  createAudioForCurrent();
}

function playGalaxyAudio() {
  if (!galaxyAudio) preloadGalaxyAudio();
  if (galaxyAudio) {
